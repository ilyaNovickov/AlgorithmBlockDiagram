...
namespace GSAApp
{
    public partial class Form1 : Form
    {
        ...
        private void zoomAndDragControl1_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (SelectedElement is Area || SelectedElement is Text)
                //Рисовать выделение
            ...
            switch (e.Button)
            {
                ...
                case MouseButtons.Right:
                    ...
                    if (diagram.GetElement(e.Location, out IDiagramElement element))
                    {
                        ...
                        if (element is Area || element is Text)
            ...
            if (e.Location != oldMousePoint)
            {
                ...
                if (SelectedElement != null)
                {
                    ...
                    if (cursorRadioButton.Checked)
                    {
                        ...
                        else//Иначе если просто нажата линия
                            //Двинуть линию 
                    ...
                    else if (resizeRadioButton.Checked)
                    {
                        ...
                        if (SelectedElement is ClassWithRect)
                        {
        ...
        private void InitilizateProperties(object element)
        {
            ...
            if (element is Area)
            {
                //Приведение к типу Area
                Area shape = (Area)element;
        ...
        private void ResizeElement(IDiagramElement element, Point point)
        {
            ...
            if (element is ClassWithRect)
            {
                //Если элемент блок-схемы - это текст с включённым авторазмером
                if (element is Text && ((Text)element).AutoSize)
                    return;//то выход из метода
                //Иницилизация размера 
                Size newSize = ((ClassWithRect)element).Size;
                ...
                ((ClassWithRect)element).Size = newSize;
                //Если размер был изменён
                if (newSize == ((ClassWithRect)element).Size)
                {
                    //то создание вспомогательной переменной положения элемента блок-схемы
                    Point newPoint = ((ClassWithRect)element).Point;
                    ...
                    ((ClassWithRect)element).Point = newPoint;
                }
                //Сохранение нового положения курсора мыши
                oldMousePoint = point;
            }
        ...
        private void CenterElement(IDiagramElement element)
        {
            ...
            //((Area)element).Point = new Point(element.Rectangle.Left - element.Rectangle.Width / 2, 
            //    element.Rectangle.Top - element.Rectangle.Height / 2);
            else if (element is Line)
        ...
        private void addPointButton_Click(object sender, EventArgs e)
        {
            ...
            if (SelectedElement is Line)
                //Вставить точку
                ((Line)SelectedElement).InsertPoint(oldMousePoint, ((Line)SelectedElement).GetAllPoints().Length - 1);
            //Вызов перерисовки элемента управления
            zoomAndDragControl1.Refresh();
        }
        ...
        private void удалитьТочкуToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (SelectedElement is Line)
            {
                ...
                int index = ((Line)SelectedElement).GetAllPoints().Length - 1;
                //Если индекс предпоследней точки линии больше 1
                if (index > 1)
                    //Удалить предпоследнюю точку линии
                    ((Line)SelectedElement).DeletePoint(index - 1);
        ...
        private void открытьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                //Если была получена ошибка, то вывод сообщения
                catch (Exception ex) 
                {
